...
@pytest.mark.parametrize(
    ...
    (
        (False, "https://external.example.com"),
        (True, "https://external.example.com"),
        (True, None),
    ),
)
def test_get_fs_endpoint(appctx, s3_storage, external, external_endpoint):
    """Test internal and external endpoint selection."""
    extension = appctx.extensions["invenio-s3"]
    fs_instance = object()
    internal_endpoint = appctx.config["S3_ENDPOINT_URL"]
    expected_endpoint = (
        external_endpoint if external and external_endpoint else internal_endpoint
    )
    with patch.dict(appctx.config), patch.dict(extension.__dict__):
        if external_endpoint:
            appctx.config["S3_EXTERNAL_ENDPOINT_URL"] = external_endpoint
        else:
        ...
        with patch(
            "invenio_s3.storage.s3fs.S3FileSystem", return_value=fs_instance
        ) as s3_filesystem:
            fs, path = s3_storage._get_fs(external=external)

            assert fs is fs_instance
            assert path == s3_storage.fileurl
            assert s3_filesystem.call_args.kwargs["client_kwargs"]["endpoint_url"] == (
                expected_endpoint
            )


def test_send_file_uses_external_filesystem(base_app, s3_storage):
    """Test download URLs are generated by the external filesystem."""
    endpoint = "https://external.example.com"
    external_fs = Mock()
    external_fs.url.return_value = f"{endpoint}/signed"

    with patch.object(
        s3_storage, "_get_fs", return_value=(external_fs, s3_storage.fileurl)
    ) as get_fs:
        with patch(
            "invenio_s3.storage.redirect_stream",
            side_effect=lambda url_builder, *args, **kwargs: url_builder(),
        ):
            with base_app.test_request_context():
                url = s3_storage.send_file("download.txt")

    assert url == f"{endpoint}/signed"
    get_fs.assert_called_once_with(external=True)
